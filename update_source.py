...
# --- 3. 기존 데이터 완벽 로드 (핵심 섹션) ---
# 기존 파일이 있다면 그 내용을 기반으로 시작합니다.
if os.path.exists(JSON_FILE):
    with open(JSON_FILE, 'r', encoding='utf-8') as f:
        try:
            base_data = json.load(f)
            print(f"📂 기존 {JSON_FILE} 데이터를 로드했습니다.")
        except Exception as e:
            print(f"⚠️ 기존 JSON 로드 실패, 새로 생성합니다: {e}")
            base_data = { "apps": [] }
else:
    base_data = { "apps": [] }

# 기본 메타데이터 강제 업데이트 (필요한 경우만)
base_data.update({
    "name": "NightFox",
...
# [정당화] 불필요한 필드 삭제 로직
if 'news' in base_data: del base_data['news']
if 'patreonURL' in base_data: del base_data['patreonURL']
...
for ipa_file in ipa_files:
    ...
    # 기존 앱 목록에서 번들 ID가 일치하는 앱 찾기
    app_entry = next((a for a in base_data.get('apps', []) if a.get('bundleIdentifier') == current_bundle_id), None)
    ...
    if app_entry:
        # [정당화] 이미 있는 앱(수동 추가 포함)은 버전과 URL만 업데이트
...
print(f"🎉 작업 완료: 수동으로 추가한 앱 정보가 보존되었습니다.")
