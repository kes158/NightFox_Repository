...
# [유지] 스포티파이 외부 소스 URL은 그대로 유지하여 동기화합니다.
...
# 사이드스토어 규격 준수: size는 Int, 나머지는 String 강제
        ...
        if response.status_code == 200:
            ...
            for ext_app in ext_apps:
                ...
                if bid not in existing_bids:
                    # 새로운 앱은 리스트 끝에 추가하여 기존 앱 순서를 유지합니다.
...
# [앱 순서 유지] base_data['apps']의 순서는 건드리지 않습니다.
for app in base_data.get('apps', []):
    ...
    if ipa_match:
        ...
        if not version_exists:
            # 새로운 버전을 배열의 맨 앞(0번 인덱스)에 삽입합니다[cite: 2].
            app["versions"].insert(0, new_v)

    # [버전 정렬] 각 앱 내의 versions 리스트만 최신순으로 정렬합니다[cite: 2].
    # 버전 번호 숫자를 비교하여 21.17.3이 21.8.3보다 위에 오도록 합니다[cite: 2].
    ...
    # 최신 버전 정보를 앱 상위 필드에 동기화합니다[cite: 2].
...
with open(JSON_FILE, 'w', encoding='utf-8') as f:
    # indent=2를 사용하여 가독성 있게 저장합니다[cite: 2].
    json.dump(base_data, f, ensure_ascii=False, indent=2)

print("🎉 NightFox.json 업데이트 완료! (Spotify 유지, 앱 순서 보존, 버전 최신순 정렬)")
